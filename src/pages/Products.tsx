...
const dispatch = useDispatch();
  ...
  const handleAddCategory = () => {
    if (categoryName.trim()) {
      dispatch(addCategory({ id: Date.now().toString(), name: categoryName.trim() }));
      setCategoryName('');
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-lg shadow-lg relative">
            <h2 className="text-xl font-semibold mb-4 text-black font-worksans">Add Category</h2>
            <label className="block text-sm text-black font-normal mb-1 font-worksans">Category Name *</label>
            <input
              type="text"
              value={categoryName}
