...
import { addVariant, removeVariant, updateVariant } from '../store/slices/productSlice';
...
const VariantsForm: React.FC<Props> = ({ showErrors = false }) => {
  ...
  const predefinedOptions = ['Size', 'Color'];
  ...
  const handleNameChange = (index: number, value: string) => {
    const updated = { ...variants[index], name: value };
    dispatch(updateVariant({ index, variant: updated }));
  };
  ...
  const handleRemoveValue = (variantIndex: number, valueIndex: number) => {
    ...
    dispatch(updateVariant({ index: variantIndex, variant: updatedVariant }));
  };
  ...
  const handleAddOption = () => {
    dispatch(addVariant({ name: '', values: [] }));
  };
    ...
    <ul className="absolute z-10 mt-1 w-full bg-white border rounded shadow text-sm">
      {predefinedOptions
        ...
        .map((option, optionIdx) => (
          <li
            key={optionIdx}
            onMouseDown={() => handleNameChange(idx, option)}
          ...
          <div className="pt-2">
            <button onClick={() => dispatch(removeVariant(idx))}>
      ...
      <button
        className="text-sm text-brand-blue font-semibold underline"
